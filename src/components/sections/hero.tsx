...
export function HeroSection() {
  return (
    <AnimatedSection id="home" className="relative min-h-screen overflow-hidden bg-[#130a18] px-5 pb-16 pt-28 text-white sm:px-8 lg:px-12 lg:pt-32">
      <div className="absolute -right-40 top-20 h-[34rem] w-[34rem] rounded-full border border-white/10" />
      <div className="absolute -right-20 top-40 h-[22rem] w-[22rem] rounded-full border border-[#d8ff63]/25" />

      <div className="relative mx-auto grid min-h-[calc(100vh-9rem)] max-w-7xl gap-8 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
        <div>
          <p className="eyebrow inline-flex rounded-full border border-white/20 px-4 py-2 text-[#d8ff63]">{profile.role}</p>
          <h1 className="mt-7 max-w-3xl text-6xl font-black leading-[0.88] tracking-[-0.065em] sm:text-7xl lg:text-[6.5rem]">
            Building ideas<br/><span className="text-[#ff5c35]">that move</span><br/>people forward.
          </h1>
          <p className="mt-7 max-w-xl text-lg leading-relaxed text-white/65">Frontend developer, product builder, and technology founder creating useful digital experiences from Gombe, Nigeria.</p>

          <div className="mt-8 flex flex-wrap gap-3">
            <a
              href="#projects"
              className="interactive rounded-xl bg-[#d8ff63] px-6 py-3.5 text-sm font-bold text-[#130a18]"
            >
        ...
        <div className="relative flex flex-col items-center lg:items-end">
          ...
          <div className="relative mt-5 w-full max-w-[430px]">
            <p className="eyebrow text-[#d8ff63]">{profile.handle}</p>
            ...
            <div className="mt-2 flex flex-wrap gap-x-4 gap-y-2 text-sm text-white/60">
              <span className="inline-flex items-center gap-1.5"><MapPin aria-hidden="true" className="h-4 w-4 text-[#d8ff63]" />{profile.location}</span>
              <span className="inline-flex items-center gap-1.5"><BriefcaseBusiness aria-hidden="true" className="h-4 w-4 text-[#d8ff63]" />5+ years building for the web</span>
            </div>
          </div>
        </div>
      </div>
